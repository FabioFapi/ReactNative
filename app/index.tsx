...
export default function Index() {
  ...
  const manageTask = useCallback((id: string) => {
    setTasks((prevState) => {
      // Cerchiamo la task con l'id corrispondente
      const taskToManage = prevState.find((task) => task.id === id);
  
      // Se la task non è stata trovata, ritorniamo lo stato precedente
      if (!taskToManage) {
        return prevState;
      }
  
      // Se la task è già completata, la rimuoviamo
      if (taskToManage.isCompleted) {
        return prevState.filter((task) => task.id !== id);
      }
  
      // Altrimenti, aggiorniamo la proprietà isCompleted
